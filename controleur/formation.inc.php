...
if (isset($_POST['envoi'])){
    if(!empty($_POST['contenu']) && !empty($_POST['date']) && !empty($_POST['nbJours']) && !empty($_POST['credit']) && !empty($_POST['prerequis'])) {
        include_once ("modele/edition.lib.php");
        $id = $_GET['formation'];
        editionFormation($id ,$_POST['contenu'], $_POST['date'], $_POST['nbJours'], $_POST['credit'], $_POST['prerequis']);
        $err = "Formation Modifié";
    }
    else{
